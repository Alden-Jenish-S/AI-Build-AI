...
class InitialAgent:
    ...
    def repair_initial_algorithm(
        ...
        system_prompt = (
            ...
            f"{fidelity} fidelity and use its fold_ids. If y_eval is None or the task "
            "is unsupervised clustering, call evaluate_clustering_predictions to "
            "write and score the clustering outputs; otherwise write complete "
...
"""
        ...
        if task_type == "unsupervised_clustering":
            evaluation_instructions = (
                ...
                f"call `prepare_evaluation_data(train_data, '{fidelity}')`. y_eval "
                ...
                "Call `evaluate_clustering_predictions(X_eval, labels, row_ids, "
                f"fold_ids, fidelity='{fidelity}')`; it must write the independently "
                "verifiable OOF and silhouette-proxy files. Fit the final clustering "
                "pipeline and predict one integer cluster for every X_test row. "
