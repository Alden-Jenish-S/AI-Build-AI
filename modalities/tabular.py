"""Tabular task discovery and profiling adapter."""
...
from .common import json_safe, records_to_bundle
...
def _resolved_target_field(task_dir: Path, layout: dict) -> str | None:
    """Resolve a target without changing the legacy layout/report contract."""
    configured = layout.get("target_column")
    train_path = _role_path(task_dir, layout.get("roles", {}).get("train"))
    if train_path is None:
        return str(configured) if configured else None
    separator = "\t" if train_path.suffix.lower() == ".tsv" else ","
    try:
        train_columns = pd.read_csv(
            train_path, sep=separator, nrows=0
        ).columns.tolist()
    except Exception:
        return str(configured) if configured else None
    if configured in train_columns:
        return str(configured)

    test_path = _role_path(task_dir, layout.get("roles", {}).get("test"))
    if test_path is not None:
        test_separator = (
            "\t" if test_path.suffix.lower() == ".tsv" else ","
        )
        try:
            test_columns = set(
                pd.read_csv(
                    test_path, sep=test_separator, nrows=0
                ).columns
            )
            train_only = [
...
def build_task_profile(
    task_dir: Path, task_spec: TaskSpec | None = None
) -> dict[str, object]:
    """Return a compact structured profile without changing legacy reports."""
    task_dir = Path(task_dir)
    layout = discover_dataset_layout(task_dir)
    if task_spec is None:
        task_spec = TabularAdapter().discover(task_dir)
    primary_role = (
        "data"
        if task_spec.problem_type == "unsupervised_clustering"
        else "train"
    )
    primary = _role_path(task_dir, layout["roles"].get(primary_role))
    profile: dict[str, object] = {
        ...
        "inventory": list(layout["inventory"]),
        "sample_count": None,
        "feature_count": None,
        "feature_types": {},
        "missing_values": {},
    ...
    if primary is None:
        profile["warnings"] = ["primary tabular input could not be resolved"]
        return profile
    separator = "\t" if primary.suffix.lower() == ".tsv" else ","
    file_size_bytes = primary.stat().st_size
    sampled = file_size_bytes > 300 * 1024 * 1024
    try:
        frame = pd.read_csv(
            primary, sep=separator, nrows=100_000 if sampled else None
        )
    except Exception as exc:
        profile["warnings"] = [f"failed to profile {primary.name}: {exc}"]
        return profile
    target_field = (
        task_spec.target.field if task_spec.target is not None else None
    )
    feature_columns = [
        column for column in frame.columns if column != target_field
    ]
    profile.update(
        {
            ...
            "missing_values": {
                str(column): int(frame[column].isnull().sum())
...
class TabularAdapter:
    """Expose current tabular behavior through modality-neutral contracts."""

    name = "tabular"

    def discover_layout(self, task_dir: Path) -> dict:
        return discover_dataset_layout(task_dir)
    ...
    def profile(
        ...
        return build_task_profile(task_dir, task_spec)
    ...
    def build_bundle(
        ...
        records = []
        for role in ("train", "data", "test"):
            relative = layout["roles"].get(role)
            if not relative or (role == "data" and records):
                continue
            path = _role_path(task_dir, relative)
            if path is None:
                continue
    ...
    def render_report(
        ...
        return run_dataset_analysis(task_dir)
